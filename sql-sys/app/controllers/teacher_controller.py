...
@teacher_router.post("/schema/update", response_model=SchemaEditResponse, summary="编辑数据库模式")
async def edit_database_schema(
    ...
    请求参数：
    - schema_id: 数据库模式ID
    - schema_description: 数据库模式描述（HTML格式）
    - schema_name: 数据库模式名称
    - schema_status: 权限信息（0为学生不可见，1为可见）
    ...
    """
    try:
        success, message, response = teacher_service.edit_database_schema(
            teacher_id=current_user["id"],
            schema_data=schema_data,
            db=db
        )
