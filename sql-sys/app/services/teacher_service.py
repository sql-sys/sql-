...
from schemas.teacher import (
    ...
    SchemaEditRequest, SchemaEditResponse,
...
class TeacherService:
    ...
    def edit_database_schema(self, teacher_id: str, schema_data: SchemaEditRequest,
                            db: Session) -> Tuple[bool, str, Optional[SchemaEditResponse]]:
        """编辑数据库模式"""
        try:
            # 验证教师是否存在
            teacher = db.query(Teacher).filter(Teacher.teacher_id == teacher_id).first()
            ...
            schema = db.query(DatabaseSchema).filter(
                DatabaseSchema.schema_id == schema_data.schema_id
            ).first()
            if not schema:
                return False, f"数据库模式ID '{schema_data.schema_id}' 不存在", None

            # 验证schema_status参数的有效性
            if schema_data.schema_status not in [0, 1]:
                return False, "状态参数无效，只能为0（学生不可见）或1（可见）", None

            # 更新数据库模式信息
            schema.schema_discription = schema_data.schema_description
            schema.schema_name = schema_data.schema_name
            schema.schema_status = schema_data.schema_status
            
            db.commit()

            response = SchemaEditResponse(
                code=200,
                msg="编辑数据库模式成功"
            )

            return True, "编辑数据库模式成功", response

        except Exception as e:
            db.rollback()
            print(f"编辑数据库模式失败: {e}")
            return False, f"编辑失败: {str(e)}", None
