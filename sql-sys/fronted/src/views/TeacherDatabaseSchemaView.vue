          ...
          <div class="right-panel" v-if="selectedSchema">
            ...
            <div v-if="activeTab === 'basic'" class="content-panel">
              ...
              <div class="basic-info-content">
                ...
                <!-- 数据库模式状态设置（仅编辑模式显示） -->
                <div v-if="isEditMode" class="schema-status-setting">
                  <h4>🔒 数据库模式状态设置</h4>
    ...
    >
      <el-form :model="createForm" :rules="createRules" ref="createFormRef" label-width="120px">
        ...
        <el-form-item label="MySQL建表文件" required>
          <el-upload
            :before-upload="(file: File) => handleFileChange(file, 'mysql')"
            :show-file-list="true"
            :limit="1"
            accept=".sql"
            drag
        ...
        <el-form-item label="PostgreSQL/OpenGauss建表文件" required>
          <el-upload
            :before-upload="(file: File) => handleFileChange(file, 'postgresql')"
            :show-file-list="true"
            :limit="1"
            accept=".sql"
            drag
...
const saveChanges = async () => {
  ...
  try {
    ...
    const response = await axios.post('/teacher/schema/update', updateData, {
      headers: {
        'Content-Type': 'application/json',
      },
    })
    ...
    if (response.data && response.data.code === 200) {
      ElMessage.success(response.data.msg || '编辑数据库模式成功')
      ...
      ElMessage.error(response.data?.msg || '编辑数据库模式失败')
    }
  } catch (error) {
    console.error('编辑数据库模式失败:', error)
    ElMessage.error('编辑数据库模式失败')
  } finally {
