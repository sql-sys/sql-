...
class StudentService:
    ...
    def get_student_profile(self, student_id: str, db: Session) -> Optional[StudentProfileResponse]:
        """获取学生个人信息"""
        try:
            # print(f"参数: student_id={student_id}")
            #调用公共服务获取当前学期id
        ...
        try:
            ...
            for student in students_in_semester:
                ...
                student_rankings.append({
                    "student_id": student.id,  # 学生表的主键id
                    "student_name": student.student_name or "未知",
                    "student_class": student.class_ or "未知班级",
            ...
            for i, student_data in enumerate(student_rankings[:limit], 1):
                result.append({
                    "名次": i,
                    "学生号": student_data["student_id"],  # 学生表的主键id
                    "姓名": student_data["student_name"],
                    "班级": student_data["student_class"],
    ...
    def get_student_dashboard(self, student_id: str, problem_id: int, db: Session) -> StudentDashboardResponse:
        ...
        try:
            ...
            # 查询题目信息，获取knowledge字段
            problem = db.query(Problem).filter(Problem.problem_id == problem_id).first()
            knowledge = problem.knowledge if problem else None
